...
async function makeAuthenticatedRequestCore(method, url, data = null, token) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Error (${method}-${url}): ${errorData.message || response.statusText}`);
    }
...
async function makeRequestCore(method, url, data = null) {
  try {
    ...
    console.error(`Error (${method}-${url}):`, error.response?.data || error.message);
    throw new Error(`Error (${method}-${url}): ${error.response?.data || error.message}`)
  }
}
...
async function getDomain() {
  try {
    ...
    const domain = domains.includes("ptct.net") ? "ptct.net" : domains[0] || null;
...
async function deleteAccount(id, token) {
  try {
    const response = await makeAuthenticatedRequest("delete", `${URL.account}/${id}`, null, token);
...
async function getMessages(token) {
  try {
    const response = await makeAuthenticatedRequest("get", URL.message, null, token);
    return response;
  }
  catch (error) {
    console.error("Failed to register account", error.message);
...
function rateLimit(fn, limitMs) {
  ...
  return async (...args) => {
    ...
    if (diff < limitMs) await new Promise(cb => setTimeout(cb, limitMs - diff))
    lastCall = Date.now()
    return fn(...args);
  }
}
